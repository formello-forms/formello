...
namespace Formello\Katzgrau\KLogger;

use DateTime;
use RuntimeException;
use Formello\Psr\Log\AbstractLogger;
use Formello\Psr\Log\LogLevel;
/**
...
class Logger extends AbstractLogger
{
     ...
     * @var array
     */
    protected $options = array('extension' => 'txt', 'dateFormat' => 'Y-m-d G:i:s.u', 'filename' => \false, 'flushFrequency' => \false, 'prefix' => 'log_', 'logFormat' => \false, 'appendContext' => \true);
    /**
     ...
     * @var array
     */
    protected $logLevels = array(LogLevel::EMERGENCY => 0, LogLevel::ALERT => 1, LogLevel::CRITICAL => 2, LogLevel::ERROR => 3, LogLevel::WARNING => 4, LogLevel::NOTICE => 5, LogLevel::INFO => 6, LogLevel::DEBUG => 7);
    /**
    ...
    public function __construct($logDirectory, $logLevelThreshold = LogLevel::DEBUG, array $options = array())
    {
        ...
        if (!\file_exists($logDirectory)) {
            \mkdir($logDirectory, $this->defaultPermissions, \true);
        }
    ...
    protected function formatMessage($level, $message, $context)
    {
        if ($this->options['logFormat']) {
            $parts = array('date' => $this->getTimestamp(), 'level' => \strtoupper($level), 'level-padding' => \str_repeat(' ', 9 - \strlen($level)), 'priority' => $this->logLevels[$level], 'message' => $message, 'context' => \json_encode($context));
    ...
    protected function contextToString($context)
    {
        ...
        foreach ($context as $key => $value) {
            $export .= "{$key}: ";
            $export .= \preg_replace(array('/=>\\s+([a-zA-Z])/im', '/array\\(\\s+\\)/im', '/^  |\\G  /m'), array('=> $1', 'array()', '    '), \str_replace('array (', 'array(', \var_export($value, \true)));
            $export .= \PHP_EOL;
        }
