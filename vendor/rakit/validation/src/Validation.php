...
use Closure;
use Formello\Rakit\Validation\Rules\Interfaces\BeforeValidate;
...
class Validation
{
     ...
     * @return void
     */
    public function __construct(\Formello\Rakit\Validation\Validator $validator, array $inputs, array $rules, array $messages = [])
    {
        $this->validator = $validator;
        $this->inputs = $this->resolveInputAttributes($inputs);
        $this->messages = $messages;
        $this->errors = new \Formello\Rakit\Validation\ErrorBag();
    ...
    public function addAttribute(string $attributeKey, $rules)
    {
        ...
        $attribute = new \Formello\Rakit\Validation\Attribute($this, $attributeKey, $this->getAlias($attributeKey), $resolvedRules);
    ...
    public function validate(array $inputs = [])
    {
        ...
        foreach ($this->attributes as $attributeKey => $attribute) {
            foreach ($attribute->getRules() as $rule) {
                if ($rule instanceof \Formello\Rakit\Validation\Rules\Interfaces\BeforeValidate) {
                    $rule->beforeValidate();
                }
            }
        }
     ...
     */
    public function errors() : \Formello\Rakit\Validation\ErrorBag
    {
        return $this->errors;
    }
    /**
     ...
     * @return void
     */
    protected function validateAttribute(\Formello\Rakit\Validation\Attribute $attribute)
    {
        ...
        foreach ($rules as $ruleValidator) {
            ...
            if ($ruleValidator instanceof \Formello\Rakit\Validation\Rules\Interfaces\ModifyValue) {
     ...
     * @return bool
     */
    protected function isArrayAttribute(\Formello\Rakit\Validation\Attribute $attribute) : bool
    {
     ...
     * @return array
     */
    protected function parseArrayAttribute(\Formello\Rakit\Validation\Attribute $attribute) : array
    {
        $attributeKey = $attribute->getKey();
        $data = \Formello\Rakit\Validation\Helper::arrayDot($this->initializeAttributeOnData($attributeKey));
        ...
        foreach ($data as $key => $value) {
            if ((bool) \preg_match('/^' . $pattern . '\\z/', $key, $match)) {
                $attr = new \Formello\Rakit\Validation\Attribute($this, $key, null, $attribute->getRules());
    ...
    protected function initializeAttributeOnData(string $attributeKey) : array
    {
        ...
        return \Formello\Rakit\Validation\Helper::arraySet($data, $attributeKey, null, \true);
    }
    /**
    ...
    public function extractValuesForWildcards(array $data, string $attributeKey) : array
    {
        ...
        $data = [];
        foreach ($keys as $key) {
            $data[$key] = \Formello\Rakit\Validation\Helper::arrayGet($this->inputs, $key);
        }
        return $data;
    }
    /**
    ...
    {
        $results = [];
        $value = \Formello\Rakit\Validation\Helper::arrayGet($this->inputs, $attributeKey, '__missing__');
        if ($value != '__missing__') {
            \Formello\Rakit\Validation\Helper::arraySet($results, $attributeKey, $value);
        }
        return $results;
    }
    /**
     ...
     * @return void
     */
    protected function addError(\Formello\Rakit\Validation\Attribute $attribute, $value, \Formello\Rakit\Validation\Rule $ruleValidator)
    {
     ...
     * @return bool
     */
    protected function ruleIsOptional(\Formello\Rakit\Validation\Attribute $attribute, \Formello\Rakit\Validation\Rule $rule) : bool
    {
        return \false === $attribute->isRequired() and \false === $rule->isImplicit() and \false === $rule instanceof \Formello\Rakit\Validation\Rules\Required;
    }
    /**
     ...
     * @return string
     */
    protected function resolveAttributeName(\Formello\Rakit\Validation\Attribute $attribute) : string
    {
     ...
     * @return mixed
     */
    protected function resolveMessage(\Formello\Rakit\Validation\Attribute $attribute, $value, \Formello\Rakit\Validation\Rule $validator) : string
    {
        $primaryAttribute = $attribute->getPrimaryAttribute();
    ...
    protected function resolveRules($rules) : array
    {
        ...
        foreach ($rules as $i => $rule) {
            ...
            } elseif ($rule instanceof \Formello\Rakit\Validation\Rule) {
                $validator = $rule;
            } elseif ($rule instanceof \Closure) {
                $validator = \call_user_func_array($validatorFactory, ['callback', $rule]);
            } else {
                $ruleName = \is_object($rule) ? \get_class($rule) : \gettype($rule);
                $message = "Rule must be a string, Closure or '" . \Formello\Rakit\Validation\Rule::class . "' instance. " . $ruleName . " given";
                throw new \Exception();
            }
    ...
    {
        return \Formello\Rakit\Validation\Helper::arrayGet($this->inputs, $key);
    }
    /**
     * Set input value
     *
    ...
    {
        \Formello\Rakit\Validation\Helper::arraySet($this->inputs, $key, $value);
    }
    /**
    ...
    {
        return \Formello\Rakit\Validation\Helper::arrayHas($this->inputs, $key);
    }
    /**
     ...
     */
    public function getValidator() : \Formello\Rakit\Validation\Validator
    {
     ...
     * @return void
     */
    protected function setValidData(\Formello\Rakit\Validation\Attribute $attribute, $value)
    {
        $key = $attribute->getKey();
        if ($attribute->isArrayAttribute() || $attribute->isUsingDotNotation()) {
            \Formello\Rakit\Validation\Helper::arraySet($this->validData, $key, $value);
            \Formello\Rakit\Validation\Helper::arrayUnset($this->invalidData, $key);
        } else {
     ...
     * @return void
     */
    protected function setInvalidData(\Formello\Rakit\Validation\Attribute $attribute, $value)
    {
        $key = $attribute->getKey();
        if ($attribute->isArrayAttribute() || $attribute->isUsingDotNotation()) {
            \Formello\Rakit\Validation\Helper::arraySet($this->invalidData, $key, $value);
            \Formello\Rakit\Validation\Helper::arrayUnset($this->validData, $key);
        } else {
