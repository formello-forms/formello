...
namespace Formello\Rakit\Validation\Rules\Traits;
...
trait FileTrait
{
    ...
    public function resolveUploadedFileValue($value)
    {
        ...
        $arrayDots = \Formello\Rakit\Validation\Helper::arrayDot($value);
        $results = [];
        foreach ($arrayDots as $key => $val) {
            ...
            \Formello\Rakit\Validation\Helper::arraySet($results, $key, $val);
        }
        return $results;
    }
}
