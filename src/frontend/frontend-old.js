...
function validate( formEl ){
  ...
  /*if( ! JSON.parse( settings.recaptcha ) ){
		submitForm( formEl )
		return
  }

  if( !formello.settings.options.recaptcha.site_key ){
		submitForm( formEl )
		return
  }

  if( formello.settings.options.recaptcha.version == 3 ){

	grecaptcha.ready(function() {
	  grecaptcha.execute( formello.settings.options.recaptcha.site_key, {action: 'submit'}).then(function(token) {

		submitForm( formEl, token )

	  });
	}, addFormMessage( formEl, { type: 'error', text: 'Invalid recaptcha' } ), loader.stop() );

  }*/

  //if( formello.settings.options.recaptcha.version == 1 ){
		submitForm( formEl )
  //}
...
var bouncer = new Bouncer('.wp-block-formello-form', {
  disableSubmit: true,
  messages: formello.settings.options.messages
})
...
window.formelloCallback = () => {
  ...
  for (var i = 0; i < buttons.length; i++) {
	...
	grecaptcha.render( gReCaptcha, {
	  'sitekey' : formello.settings.options.recaptcha.site_key
	});
